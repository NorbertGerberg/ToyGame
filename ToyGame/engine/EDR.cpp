...
zip* EDR::OpenZIPArchive(const char* directory)
{
	//Open ZIP archive
	int err = 0;
...
zip* EDR::CreateZIPArchive(const char* directory)
{
	//Open ZIP archive
	int err = 0;
...
void EDR::AddFileToZIP(zip* z, const char* filename, const char* content)
{
	zip_source_t* s;
...
void EDR::CopyFileToZIP(zip* z, const char* filename, const char* filepath)
{
	zip_source_t* imageSource = zip_source_file(z, filepath, 0, 0);
	zip_file_add(z, filename, imageSource, ZIP_FL_ENC_UTF_8);
}

void EDR::RemoveFileFromZIP(zip* z, const char* filename)
{
...
EDR::ZSTAT EDR::GetZIPFileContents(zip* z, const char* name)
{
	//Search for the file
	struct zip_stat st;
	zip_stat_init(&st);
...
void EDR::CopyZIPFileContents(zip* z, const char* name, const char* newPath, const bool isBinary)
{
	//Search for the file
	struct zip_stat st;
	zip_stat_init(&st);
...
strg EDR::CopyZIPFileContents(zip* z, const char* name)
{
	//Search for the file
	struct zip_stat st;
	zip_stat_init(&st);
